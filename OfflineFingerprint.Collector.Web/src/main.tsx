...
function App(){
  ...
  function selectCell(f:string,p:'left'|'center'|'right'){
    if(live){
      if(f===finger&&p===position&&latestGrayRef.current){saveLatest();return}
      setErr(`กำลังสแกน ${finger} / ${positionLabel(position)} อยู่ กรุณาบันทึกหรือยกเลิกก่อนเปลี่ยนตำแหน่ง`);return
    }

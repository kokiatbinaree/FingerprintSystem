...
function ZoomBox({children,className='',resetKey}:ZoomBoxProps){
  ...
  function onWheel(e:React.WheelEvent){e.preventDefault();const el=ref.current;if(!el)return;const rect=el.getBoundingClientRect();const px=e.clientX-rect.left;const py=e.clientY-rect.top;const next=Math.min(5,Math.max(1,scale*(e.deltaY<0?1.18:1/1.18)));if(next===scale)return;setPan({x:px-(px-pan.x)*(next/scale),y:py-(py-pan.y)*(next/scale)});setScale(next)}
  function onDoubleClick(){setScale(1);setPan({x:0,y:0})}
  return <div ref={ref} className={`zoom-box ${className}`} onWheel={onWheel} onDoubleClick={onDoubleClick}><div className="zoom-layer" style={{transform:`translate(${pan.x}px,${pan.y}px) scale(${scale})`}}>{children}</div><div className="zoom-badge">{Math.round(scale*100)}%</div></div>
}
...
function App(){
  ...
  useEffect(()=>{
    let cancelled=false;const old=gallery;old.forEach(x=>URL.revokeObjectURL(x.url));setGallery([]);setSelectedImage(null);if(!selectedPerson||!finger)return;
  ...
  function selectCell(f:string,p:'left'|'center'|'right'){if(live)return;setFinger(f);setPosition(p);setSelectedImage(null);setNotice('')}
  ...
  function startLive(){if(!selectedPerson)return setErr('กรุณาเลือก Person ก่อน');if(live)return;setErr('');setNotice('');latestGrayRef.current='';const ws=new WebSocket(AGENT_WS);ws.binaryType='blob';wsRef.current=ws;setScannerBusy(false);setSelectedImage(null);setLive({width:320,height:480,grayBase64:'',running:true});ws.onmessage=e=>{if(typeof e.data==='string')return;drawBlob(e.data as Blob)};ws.onerror=()=>{setErr('เชื่อมต่อ FutronicBridge ไม่สำเร็จ กรุณาเปิด Collector.Agent :15271');cancelLive(true)};ws.onclose=()=>{wsRef.current=null}}
  async function saveLatest(){if(!selectedPerson||!live)return;const grayBase64=latestGrayRef.current||live.grayBase64;if(!grayBase64){setErr('ยังไม่ได้รับภาพจาก Futronic FS80H');return}const ws=wsRef.current;if(ws){try{ws.close()}catch{}wsRef.current=null}setScannerBusy(true);setNotice('กำลังบันทึกภาพล่าสุด...');setErr('');try{await api('/api/capture/confirm',{method:'POST',body:JSON.stringify({personId:selectedPerson.id,fingerCode:finger,position,width:live.width,height:live.height,grayBase64})},token);latestGrayRef.current='';setLive(null);await loadFingerprints(selectedPerson.id);setNotice(`บันทึก ${finger} / ${positionLabel(position)} สำเร็จ`)}catch(x){setErr(String(x))}finally{setScannerBusy(false)}}
  ...
  return <div className="app-shell">
    ...
    {view==='persons'?<main className="page persons-page">
      ...
      <div className="collection-top"><div className="person-heading"><button className="back-button" onClick={()=>{cancelLive(true);setView('persons')}}>← ลูกค้า</button><div><div className="collection-title">เก็บลายนิ้วมือ</div><div className="person-identity"><b>{selectedPerson?.personCode}</b><span>{selectedPerson?personName(selectedPerson):''}</span>{selectedPerson?.nationalId&&<span>เลขบัตร {selectedPerson.nationalId}</span>}</div></div></div><div className="collection-actions"><button className="secondary" onClick={()=>selectedPerson&&setEditing(selectedPerson)}>แก้ไขข้อมูลลูกค้า</button><span className="scanner-status"><i className={`status-dot ${live?'scanning':'saved'}`}/>{live?'กำลัง Realtime':'พร้อมใช้งาน'}</span></div></div>
      {(err||notice)&&<div className={err?'banner error':'banner success'}>{err||notice}</div>}
      <section className="capture-workspace">
        <div className="capture-column preview-column"><div className="panel-title"><span>3.1 Preview</span><span className="panel-hint">Realtime / ภาพที่เลือก</span></div><div className="preview-shell">{live?<ZoomBox className="main-zoom" resetKey="live"><canvas ref={canvasRef} width={320} height={480}/>{!live.grayBase64&&<div className="preview-empty">กำลังรอภาพจาก Futronic FS80H...</div>}</ZoomBox>:selectedImage?<ZoomBox className="main-zoom" resetKey={selectedImage.id}><img src={selectedImage.url} alt="Fingerprint preview"/></ZoomBox>:<div className="preview-empty large">เลือกรูปจาก Gallery หรือกด Scan</div>}</div><div className="preview-caption">{live?'Realtime • Invert • 320 × 480':selectedImage?`${selectedImage.fingerCode} / ${positionLabel(selectedImage.position)} / #${selectedImage.sequenceNo}`:'ยังไม่ได้เลือกภาพ'}</div><div className="preview-controls"><button className="primary" disabled={!!live||scannerBusy} onClick={startLive}>เริ่ม Scan</button><button className="save-latest" disabled={!live?.grayBase64||scannerBusy} onClick={saveLatest}>✓ บันทึกภาพล่าสุด</button><button className="secondary" disabled={!live||scannerBusy} onClick={()=>cancelLive(false)}>ยกเลิก</button></div></div>
        <div className="capture-column gallery-column"><div className="panel-title"><span>3.2 Gallery</span><span className="panel-hint">{finger} • ภาพทั้งหมดของนิ้วนี้</span></div><div className="gallery-summary"><b>{finger} · {fingerLabels[finger]}</b><span>{gallery.length} ภาพ</span></div><div className="gallery-list">{galleryLoading&&<div className="gallery-loading">กำลังโหลดรูป...</div>}{!galleryLoading&&gallery.length===0&&<div className="gallery-empty">ยังไม่มีภาพของนิ้วนี้</div>}{gallery.map(g=><button key={g.id} className={selectedImage?.id===g.id?'gallery-item selected':'gallery-item'} onClick={()=>{if(!live)setSelectedImage(g)}}><img src={g.url} alt={`${g.fingerCode} ${g.position}`}/><div className="gallery-meta"><b>{positionLabel(g.position)}</b><span>#{g.sequenceNo}</span><small>{new Date(g.capturedAtUtc).toLocaleString('th-TH')}</small></div></button>)}</div></div>
        <div className="capture-column table-column"><div className="panel-title"><span>3.3 ตารางเก็บลายนิ้วมือ</span><span className="panel-hint">คลิกช่องเพื่อเลือก Finger + Position</span></div><div className="selection-note"><span>กำลังเลือก</span><b>{finger} · {fingerLabels[finger]}</b><em>ตำแหน่ง: {positionLabel(position)}</em></div><div className="finger-table-wrap"><table className="finger-table"><thead><tr><th>นิ้ว</th>{positions.map(([v,l])=><th key={v}>{l}</th>)}</tr></thead><tbody>{fingers.map(f=><tr key={f} className={finger===f?'finger-row-active':''}><td className="finger-label"><b>{f}</b><span>{fingerLabels[f]}</span></td>{positions.map(([p,l])=>{const n=cellCounts[`${f}|${p}`]||0;const active=finger===f&&position===p;return <td key={p}><button className={`finger-cell ${active?'active':''} ${n>0?'has-images':''}`} onClick={()=>selectCell(f,p as any)} disabled={!!live||scannerBusy}><span>{n}</span>{n>0&&<small>ภาพ</small>}</button></td>})}</tr>)}</tbody></table></div><div className="table-legend"><span><i className="legend-dot selected-dot"/> ช่องที่เลือก</span><span><i className="legend-dot saved-dot"/> มีภาพแล้ว</span></div></div>
      </section>
    </main>}

(() => {
  ...
  function filterFingerprintList(personId,items){
    ...
    return items.filter(item=>!deleted.some(d=>{
      const same=targetKey(personId,item.fingerCode,item.position,item.sequenceNo)===d.key;
  ...
  function svgPlaceholder(){
    return new Response('<svg xmlns="http://www.w3.org/2000/svg" width="320" height="480"><rect width="100%" height="100%" fill="white"/></svg>',{
      status:200,
  ...
  window.fetch=async(...args)=>{
    ...
    try{
      ...
      if(previewMatch){
        const deleted=readDeleted().some(x=>x.imageId===previewMatch[1]);
        if(deleted)return response.ok?response:svgPlaceholder();
      }
    }catch{}
  ...
  function css(){
    ...
    s.textContent=`
      ...
      .fp-head{padding:18px 20px 10px;display:flex;gap:12px}.fp-icon{width:42px;height:42px;border-radius:12px;display:grid;place-items:center;background:#fff0ed;color:#a2281b;font-size:20px}.fp-title{font-size:17px;font-weight:800;color:#172033}.fp-text{margin-top:4px;font-size:13px;color:#667085;line-height:1.5}.fp-actions{display:flex;justify-content:flex-end;gap:8px;padding:16px 20px 20px}.fp-cancel,.fp-confirm{border-radius:9px;padding:9px 14px;font:inherit;font-weight:700;cursor:pointer}.fp-cancel{border:1px solid #d0d7e2;background:#fff;color:#344054}.fp-confirm{border:1px solid #b42318;background:#b42318;color:#fff}.fp-confirm:disabled{opacity:.6;cursor:not-allowed}
    `;
  ...
  async function currentPerson(){
    ...
    const p=Array.isArray(a)?a.find(x=>x.personCode===code):null;
    if(!p)throw new Error('ไม่พบ Person ปัจจุบัน');
    return p;
  }
  ...
  function countsFromRows(rows){
    const out={};
    for(const x of rows||[]){
      const k=`${x.fingerCode}|${x.position}`;
      out[k]=(out[k]||0)+1;
    }
    return out;
  }
  ...
  function syncTableFromCache(){
    ...
    document.querySelectorAll('.finger-table tbody tr').forEach(row=>{
      ...
      const personId=document.querySelector('.person-identity b')?.dataset?.personId||'';
      for(let i=0;i<cells.length;i++){
        const pos=['left','center','right'][i];
        const key=personId?`${personId}|${finger}|${pos}`:`${finger}|${pos}`;
        const direct=counts[key];
        const fallback=counts[`${finger}|${pos}`];
        const value=direct!==undefined?direct:fallback;
  ...
  async function refreshCountCache(personId){
    const rows=await currentFingerprintRows(personId);
    const actual=countsFromRows(rows);
    const old=readCounts();
    const merged={...old};
    for(const k of Object.keys(merged))if(k.startsWith(`${personId}|`))delete merged[k];
    for(const [k,v] of Object.entries(actual))merged[`${personId}|${k}`]=v;
    writeCounts(merged);
    syncTableFromCache();
  }

  function meta(item){
    const sub=item.closest('.gallery-subcolumn');
    const pl=sub?.querySelector('.gallery-subtitle')?.textContent?.trim()||'';
    const pos={ซ้าย:'left',กลาง:'center',ขวา:'right'}[pl];
    const seq=Number(((item.querySelector('.gallery-meta b')?.textContent||'').match(/\d+/)||['0'])[0]);
    const alt=item.querySelector('img')?.alt||'';
    const top=document.querySelector('.gallery-top strong')?.textContent||'';
    const finger=(alt.match(/\b(?:L|R)[1-5]\b/)||top.match(/\b(?:L|R)[1-5]\b/)||[''])[0];
    return{finger,pos,pl,seq}
  }
  ...
  function hideDeletedGalleryItems(){
    ...
    for(const item of document.querySelectorAll('.gallery-item')){
      const m=meta(item);
      if(!m.finger||!m.pos||!m.seq)continue;
      const hit=deleted.some(d=>d.fingerCode===m.finger&&d.position===m.pos&&d.sequenceNo===m.seq && (!code || d.personCode===code));
      if(hit)item.classList.add('fp-hidden-deleted');
    }
  }
  ...
  function show(m,onConfirm){
    ...
    const b=modal.querySelector('.fp-confirm');
    b.onclick=async()=>{b.disabled=true;b.textContent='กำลังลบ...';try{await onConfirm();close()}catch(e){b.disabled=false;b.textContent='ยืนยันการลบ';alert(String(e))}};
    modal.addEventListener('click',e=>{if(e.target===modal)close()})
  }

  function bind(){
    css();
    hideDeletedGalleryItems();
    syncTableFromCache();
    document.querySelectorAll('.gallery-item').forEach(item=>{
      ...
      b.onclick=async e=>{
        ...
        const m=meta(item);if(!m.finger||!m.pos||!m.seq)return;
        show(m,async()=>{
          const person=await currentPerson();
          const rows=await currentFingerprintRows(person.id);
          const target=rows.find(x=>x.fingerCode===m.finger&&x.position===m.pos&&x.sequenceNo===m.seq);
          if(!target)throw new Error('ไม่พบภาพลายนิ้วมือที่ต้องการลบ');
          ...
          const actual=rows.filter(x=>x.id!==target.id);const counts=readCounts();
          for(const k of Object.keys(counts))if(k.startsWith(`${person.id}|`))delete counts[k];
          for(const[k,v]of Object.entries(countsFromRows(actual)))counts[`${person.id}|${k}`]=v;
          writeCounts(counts);
          item.classList.add('fp-removing');
